...
class AvroRowTest extends AnyFunSuite {
  ...
  private[this] final def createUnionField(name: String, schema: Schema): Schema.Field = {
    val unionSchema = Schema.createUnion(Schema.create(Schema.Type.NULL), schema)
